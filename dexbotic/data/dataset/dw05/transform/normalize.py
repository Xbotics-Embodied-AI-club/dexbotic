...
def _norm_stats_to_dict(s: NormStats) -> dict:
    return {
...
def _dict_to_norm_stats(d: dict) -> NormStats:
    return NormStats(**{k: np.array(v) for k, v in d.items()})


def serialize_json(norm_stats: dict[str, NormStats]) -> str:
    return json.dumps({"norm_stats": {k: _norm_stats_to_dict(v) for k, v in norm_stats.items()}}, indent=2)


def deserialize_json(data: str) -> dict[str, NormStats]:
    return {k: _dict_to_norm_stats(v) for k, v in json.loads(data)["norm_stats"].items()}
