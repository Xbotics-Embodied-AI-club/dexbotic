...
import dexbotic.data.normalize as normalize
from dexbotic.constants_dm05.robot import ROBOT_STATE_DESCS, ActionMode, RobotType
from dexbotic.data.augmentations import NoAugmentationPipeline, TrainingTransformPipeline
from dexbotic.data.collator import NormStatsCollator, TrainingCollator
...
from dexbotic.data.dataset_dm05.register import CONVERSATION_DATA
...
from dexbotic.optimizer_dm05 import MuonAdamW, mark_muon_parameters
from dexbotic.optimizer_dm05.muon_adamw import is_default_muon_parameter
from dexbotic.trainer_dm05.trainer import DMTrainer, safe_save_model_for_hf_trainer
...
class DM05InferenceConfig(Config):
    ...
    def _initialize(
        ...
        if self.backend == "fast":
            from dexbotic.infer_dm05.dm05_infer import DM05FastInferRuntime
            from dexbotic.infer_dm05.dm05_trt_utils import ensure_dm05_fast_inference_engine

            dm05_model = unwrap_dm05_model(model)
            num_vision_images = len(self.image_prompts)
            if is_history:
                from dexbotic.infer_dm05.dm05_trt_utils import MAX_HISTORY_IMAGES
