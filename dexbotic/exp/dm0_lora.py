"""DM0 LoRA helpers for LIBERO SFT experiments."""
...
def resolve_dm0_tokenizer_path(
...
class DM0LoraConfig(Config):
    ...
    cast_trainable_to_model_dtype: bool = field(default=True)
...
def apply_lora_to_dm0_model(
    model: DM0ForCausalLM,
    lora_config: DM0LoraConfig,
    ...
    logger.info(
        "Enabled DM0 LoRA: r={}, alpha={}, dropout={}, target_modules={}, exclude_modules={}, modules_to_save={}",
        lora_config.r,
...
def load_dm0_model_for_inference(
    model_name_or_path: str,
    base_model_name_or_path: str | None = None,
    **from_pretrained_kwargs,
) -> torch.nn.Module:
    if not is_lora_checkpoint(model_name_or_path):
        return DM0ForCausalLM.from_pretrained(
            model_name_or_path,
            **from_pretrained_kwargs,
        )
    ...
    try:
        ...
        raise ImportError("Loading a LoRA DM0 checkpoint requires `peft`") from exc
    ...
    logger.info("Loading DM0 LoRA base model from {}", base_model_path)
    base_model = DM0ForCausalLM.from_pretrained(
        base_model_path,
        **from_pretrained_kwargs,
    )
    logger.info("Loading DM0 LoRA adapter from {}", model_name_or_path)
    model = PeftModel.from_pretrained(base_model, model_name_or_path)
    logger.info("Merging DM0 LoRA adapter into the base model for inference")
