...
#
# Ported from Isaac-GR00T (gr00t/model/modules/qwen3_backbone.py) for GR00T N1.7
# ("gr00tsonic") into Dexbotic. The Qwen3-VL (Cosmos-Reason2-2B) backbone is kept
# monolithic, exactly as in the upstream model. The only addition over upstream is
# an ``init_pretrained`` switch so the backbone can be instantiated from config
# (for loading a full Dexbotic checkpoint) instead of always downloading the base
# weights with ``from_pretrained``.
...
class Qwen3Backbone(torch.nn.Module):
    def __init__(
        ...
        Args:
            ...
            init_pretrained: if True load the Cosmos-Reason2-2B weights via
                ``from_pretrained``; if False build an empty model from its config
                (weights are expected to come from a Dexbotic checkpoint afterwards).
        """
        ...
        if use_flash_attention:
            try:
                ...
                # Flash-Attention 2 requires fp16/bf16. Build the (frozen) backbone
                # directly in bf16 — the trainer casts it to bf16 anyway — so we
                # avoid the "Flash Attention 2 without specifying a torch dtype"
                # warning and the wasted fp32 allocation.
