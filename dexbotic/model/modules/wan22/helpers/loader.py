...
def _single_existing_file(candidates: list[Path], glob_patterns: list[str] | None = None) -> str | None:
    """Return the first explicit file or unambiguous glob match under a local bundle."""
...
def _bundle_tokenizer_dir(root: Path) -> str | None:
    tokenizer_dir = root / "tokenizer"
    required = ["tokenizer_config.json", "tokenizer.json", "spiece.model"]
    if tokenizer_dir.is_dir() and any((tokenizer_dir / name).is_file() for name in required):
        return str(tokenizer_dir)
    return None


def _resolve_dw05_bundle_configs(root: str | os.PathLike | None) -> tuple[ModelConfig | None, ModelConfig | None, ModelConfig | None, ModelConfig | None]:
    """Resolve DW05's user-facing local bundle layout.

    A released DW05 bundle should expose method-owned directories such as
    ``vae/``, ``text_encoder/``, and ``tokenizer/`` rather than upstream cache
    paths.  Direct paths are used when present; missing components fall back to
    the regular model-cache resolver.
    """
    if not root:
    ...
    dit_path = _single_existing_file(
        candidates=[],
        glob_patterns=[str(root_path / "video_dit" / "diffusion_pytorch_model*.safetensors")],
    )
    text_path = _single_existing_file(
        candidates=[
            root_path / "text_encoder" / "model.pth",
            root_path / "text_encoder" / "model.safetensors",
            root_path / "text_encoder" / "models_t5_umt5-xxl-enc-bf16.pth",
            root_path / "text_encoder" / "models_t5_umt5-xxl-enc-bf16.safetensors",
        ],
        glob_patterns=[str(root_path / "text_encoder" / "*.pth"), str(root_path / "text_encoder" / "*.safetensors")],
    )
    vae_path = _single_existing_file(
        candidates=[
            root_path / "vae" / "model.pth",
            root_path / "vae" / "model.safetensors",
            root_path / "vae" / "Wan2.2_VAE.pth",
            root_path / "vae" / "Wan2.2_VAE.safetensors",
        ],
        glob_patterns=[str(root_path / "vae" / "*.pth"), str(root_path / "vae" / "*.safetensors")],
    )
    tokenizer_path = _bundle_tokenizer_dir(root_path)

    return (
        ModelConfig(path=dit_path) if dit_path else None,
        ModelConfig(path=text_path) if text_path else None,
        ModelConfig(path=vae_path) if vae_path else None,
        ModelConfig(path=tokenizer_path) if tokenizer_path else None,
    )


def _local_bundle_root() -> str | None:
    return os.environ.get("DW05_MODEL_BASE_PATH") or os.environ.get("DIFFSYNTH_MODEL_BASE_PATH")
...
def _resolve_configs(model_id: str, tokenizer_model_id: str, redirect_common_files: bool = True):
    ...
    bundle_dit, bundle_text, bundle_vae, bundle_tokenizer = _resolve_dw05_bundle_configs(_local_bundle_root())
    if bundle_dit is not None:
        dit_config = bundle_dit
    if bundle_text is not None:
        text_config = bundle_text
    if bundle_vae is not None:
        vae_config = bundle_vae
    if bundle_tokenizer is not None:
        tokenizer_config = bundle_tokenizer
    ...
    if redirect_common_files:
        ...
        if text_config.path is None and text_config.origin_file_pattern in redirect_dict:
            text_config.model_id, text_config.origin_file_pattern = redirect_dict[text_config.origin_file_pattern]
        if vae_config.path is None and vae_config.origin_file_pattern in redirect_dict:
