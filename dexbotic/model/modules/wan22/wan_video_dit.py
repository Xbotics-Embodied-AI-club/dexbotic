...
class WanVideoDiT(torch.nn.Module):
    ...
    def _validate_forward_inputs(
        ...
        action_condition_start_latent: int = 1,
        ...
        if self.action_conditioned:
            if action is not None:
                ...
                action_condition_start_latent = int(action_condition_start_latent)
                if action_condition_start_latent < 1 or action_condition_start_latent >= num_latent_frames:
                    raise ValueError(
                        "`action_condition_start_latent` must be in [1, num_latent_frames-1] "
                        f"for action-conditioned video, got {action_condition_start_latent} with "
                        f"num_latent_frames={num_latent_frames}."
                    )
                num_action_groups = num_latent_frames - action_condition_start_latent
                if action.shape[1] % num_action_groups != 0:
                    raise ValueError(
                        "action horizon must be divisible by the number of action-conditioned latent groups, "
                        f"got action_horizon={action.shape[1]}, num_action_groups={num_action_groups}."
                    )
    ...
    def pre_dit(
        ...
        if self.seperated_timestep and fuse_vae_embedding_in_latents:
            ...
            clean_latent_count = int(clean_latent_count)
            if clean_latent_count < 1 or clean_latent_count > x.shape[2]:
                raise ValueError(
                    f"`clean_latent_count` must be in [1, {x.shape[2]}], got {clean_latent_count}."
                )
            ...
            token_timesteps[:, :clean_latent_count, :] = 0
        ...
        if self.action_conditioned and action is not None:
            ...
            # new mask
            action_condition_start_latent = int(action_condition_start_latent)
            if action_condition_start_latent < 1 or action_condition_start_latent >= f:
                raise ValueError(
                    "`action_condition_start_latent` must be in [1, f-1] when `action` is provided, "
                    f"got {action_condition_start_latent} with f={f}."
                )
            num_temporal_groups = f - action_condition_start_latent
            if num_temporal_groups <= 0:
                raise ValueError(
                    "Action-conditioned context mask requires at least one action-conditioned latent group."
                )
            ...
            # latent frames from action_condition_start_latent onward attend to action tokens
            action_query_start = action_condition_start_latent * tokens_per_frame
            final_context_mask[:, action_query_start:, context_len:] = action_group_mask.unsqueeze(0).expand(batch_size, -1, -1) # (B, query_len, action_len)
            context_mask = final_context_mask
        elif self.action_conditioned and action is None:
            # action dropped (CFG training) or inference without action — treat as unconditioned
