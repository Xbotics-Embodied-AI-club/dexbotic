...
from dexbotic.data.data_source.register import CONVERSATION_DATA, register_dataset
...
DOS_W1_DATASET = {
    "stack_bowls": {
        "annotations": os.environ.get("DOSW1_DEXDATA_DIR", "./data/dosw1/dexdata/stack_bowls"),
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="DexData task directory, e.g. /path/to/dexdata/stack_bowls.",
    )
    parser.add_argument(
        "--model_name_or_path",
        type=str,
        default=None,
        help="Base model for training or checkpoint for inference.",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
...
class DM0TrainerConfig(_DM0TrainerConfig):
    wandb_project: str = field(default="dm0_sft_dosw1")
    ...
    output_dir: str = field(
        default=os.environ.get(
            "DOSW1_OUTPUT_DIR",
            f"./user_checkpoints/dexbotic/dosw1_dm0/stack_bowls-{datetime.now().strftime('%m%d')}",
        )
    )
...
class DM0DataConfig(_DM0DataConfig):
    dataset_name: str = field(default="dosw1_stack_bowls")
...
class DM0ModelConfig(_DM0ModelConfig):
    model_name_or_path: str = field(
        default=os.environ.get("DM0_MODEL_NAME_OR_PATH", "Dexmal/DM0-base")
    )
...
class DM0InferenceConfig(_DM0InferenceConfig):
    model_name_or_path: Optional[str] = field(
        default=os.environ.get(
            "DM0_INFERENCE_MODEL_PATH",
            "./user_checkpoints/dexbotic/dosw1_dm0/stack_bowls",
        )
    )
    ...
    def _initialize_inference(self) -> None:
        checkpoint_norm_file = os.path.join(self.model_name_or_path, "norm_stats.json")
        if self.norm_stats is None and not megfile.smart_exists(checkpoint_norm_file):
            norm_config = DM0ComputeNormActionConfig()
            save_name = hashlib.md5(DM0DataConfig.dataset_name.encode()).hexdigest()[:8]
            self.norm_stats = self.read_normalization_stats(
                os.path.join(
                    os.path.dirname(norm_config.norm_save_path),
                    save_name,
                    "norm_stats.json",
                )
            )
...
if __name__ == "__main__":
    ...
    if args.data_dir is not None:
        CONVERSATION_DATA["dosw1_stack_bowls"]["annotations"] = args.data_dir
