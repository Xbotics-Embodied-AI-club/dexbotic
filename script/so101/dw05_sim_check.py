...
上游代码不改。换错不报错，只会把关节喂进错的槽位 —— 所以直接从训练入口导入，不抄一份。

用法（仓根，训练侧环境）：
    CUDA_VISIBLE_DEVICES=0 $PY script/so101/dw05_sim_check.py --checkpoint <weights/step_xxxxxx.pt> \\
...
from playground.dw05_so101_exp import (
    DW05_NORM_STATS,
    SO101_NON_DELTA_MASK,
    SO101_STATE_ARRANGEMENT,
)
from script.so101.layout import DW05_BUNDLE
from script.so101.rollout_so101 import read_frames
