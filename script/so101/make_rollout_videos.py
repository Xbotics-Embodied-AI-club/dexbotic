"""把 rollout_so101.py 存下的单局录像做成 top | wrist 并排的片子，顶上横幅写明来历。

横幅写清哪个模型、哪个存点、哪个场景、第几局、成没成、是策略在仿真里跑的 ——
没有这行字，策略 rollout 与数据集里同样两路并排的演示录像肉眼分不开。

用法（仓根，仿真侧环境）：
    $SIM_PY script/so101/make_rollout_videos.py --rollout-dir <评测输出下的场景目录> \\
        --out <目录> [--per-kind 2]
"""
...
from rollout_so101 import read_frames
...
def main() -> int:
    ...
    for kind in ("success", "fail"):
        tops = sorted(p for p in (args.rollout_dir / "videos").glob(f"*_{kind}.mp4"))[
            : args.per_kind
        ]
        for top_path in tops:
            ...
            label, _, rest = top_path.stem.partition(
                "_"
            )  # dm05-checkpoint-5000_cube40_ep003_success
            ...
            frames = [
                np.concatenate([head, np.concatenate([top[t], wrist[t]], axis=1)], axis=0)
                for t in range(n)
            ]
